...
(() => {
  ...
  function renderPrimary(data, label) {
    ...
    PRAYER_ORDER.forEach((key) => {
      ...
      value.textContent = parsed ? `${String(parsed.h).padStart(2, '0')}:${String(parsed.m).padStart(2, '0')}` : '—';
  ...
  function notifyPrayer(key) {
    ...
    try {
      const notification = new Notification(`حان الآن وقت أذان ${PRAYER_LABELS[key]} 🕌`, {
        body: primaryTimings ? `${els.heroLocation.textContent} — ${new Date().toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}` : '',
  ...
  function checkAdhanTime() {
    ...
    ADHAN_PRAYERS.forEach((key) => {
      ...
      if (!parsed) return;
      const timeLabel = `${String(parsed.h).padStart(2, '0')}:${String(parsed.m).padStart(2, '0')}`;
  ...
  async function renderCompareCard(entry) {
    ...
    try {
      ...
      ADHAN_PRAYERS.forEach((key) => {
        ...
        cell.innerHTML = `<span>${PRAYER_LABELS[key]}</span><b>${parsed ? `${String(parsed.h).padStart(2, '0')}:${String(parsed.m).padStart(2, '0')}` : '—'}</b>`;
      ...
      if (next) {
        ...
        nextLabel.textContent = `القادمة: ${PRAYER_LABELS[next.key]} — بعد ${arabicNumber(h)} س ${arabicNumber(m)} د`;
      } else {
