...
- عند الفتاوى الشخصية أو المسائل التي يترتب عليها طلاق، زواج، ميراث، كفارات، معاملات مالية، أو ضرر، قدّم توجيهاً عاماً فقط ووجّه المستخدم إلى عالم موثوق أو جهة إفتاء معتمدة.
...
أسلوبك: ابدأ بالجواب المباشر، ثم وضّح السبب عند الحاجة. استخدم العربية الطبيعية، وعناوين قصيرة عند الحاجة. إذا لم تجد دليلاً كافياً، كن صريحاً بذلك.`;
...
function buildPrompt(question, context = '', history = []) {
  ...
  return `سياق المحادثة السابقة:\n${conversation}\n\nالسؤال الحالي: ${question}${context ? `\nسياق إضافي: ${context}` : ''}\n\nمصادر أحاديث محلية مرتبطة بالسؤال، للاسترشاد فقط:\n${sources}\n\nأجب اعتماداً على معرفتك العامة وعلى المصادر المحلية عندما تكون مناسبة. لا تدّع أن المصدر المحلي يثبت حكماً لمجرد أنه ظهر في البحث. إذا احتجت نسبة حديث بعينه ولم يكن النص أو صحته واضحين، صرّح بذلك.`;
}
