(function () {
  ...
  function initSnake(overlay) {
    ...
    function resetGame() {
      // Build the fresh round state FIRST. Only once snake/food/direction
      // are valid do we size the canvas and draw — resize()/draw() used to
      // run before this state existed, which threw on every new round
      // (snake/food were undefined) and left the game stuck/broken.
      ...
      resize(); // sizes the canvas and draws the now-valid state
    ...
    function step() {
      ...
      directionLocked = false;
      const head = snake[0];
      // Classic wrap-around: leaving one edge enters from the opposite edge.
      const next = {
        x: (head.x + dir.x + cols) % cols,
        y: (head.y + dir.y + rows) % rows
      };
      const willEat = next.x === food.x && next.y === food.y;
      // When not eating, the tail cell vacates this same move, so moving
      // into it is legal — only check against the body that stays put.
